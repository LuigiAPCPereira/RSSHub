...
const normalizeHandler = (mod) => {
    // ESM default common case
    if (mod && typeof mod === 'object' && 'default' in mod) {
        mod = mod.default;
    }

    // If module exports an object with a `.handler` function, use it
...
const lazyloadRouteHandler = (routeHandlerPath) => async (ctx, next) => {
    // If we've already cached a normalized function, call it
    if (RouterHandlerMap.has(routeHandlerPath)) {
        const fn = RouterHandlerMap.get(routeHandlerPath);
        return fn(ctx, next);
    }

    // Load the module (keep require for existing codebase)
    let mod;
    try {
        mod = require(routeHandlerPath);
    } catch (error) {
        // Log and rethrow so caller sees a clear error when a route module is missing
        // eslint-disable-next-line no-console
        console.error(`Failed to require route handler ${routeHandlerPath}:`, error);
        throw error;
    }

    const handlerFn = normalizeHandler(mod);
    if (!handlerFn) {
        const type = mod === null ? 'null' : typeof mod;
        const extra = mod && typeof mod === 'object' ? JSON.stringify(Object.keys(mod)) : '';
        throw new Error(`Invalid route handler exported from ${routeHandlerPath} (type=${type}) ${extra}`);
    }

    RouterHandlerMap.set(routeHandlerPath, handlerFn);
    return handlerFn(ctx, next);
};
